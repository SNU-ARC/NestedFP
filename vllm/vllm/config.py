...
class CompilationConfig(BaseModel):
    ...
    def from_cli(cls, cli_value: str) -> "CompilationConfig":
        ...
        if cli_value in ["0", "1", "2", "3"]:
            return cls(level=int(cli_value))
        # Hardcoded compilation config
        return cls(
            level=3,
            custom_ops=["none"],
            splitting_ops=["vllm.unified_attention", "vllm.unified_attention_with_output"],
            use_inductor=True,
            compile_sizes=[512, 256, 128, 64, 32, 16, 8, 4, 2, 1],
            use_cudagraph=True,
            cudagraph_num_of_warmups=2,
            cudagraph_capture_sizes=[512, 256, 128, 64, 32, 16, 8, 4, 2, 1]
        )
    ...
    def init_with_cudagraph_sizes(self,
        ...
        #nestedfp
        #self.compile_sizes = [512, 256, 128, 64, 32, 16, 8, 4, 2, 1]
...
class VllmConfig:
    ...
    def __post_init__(self):
        ...
        # hardcoded_sizes = [512, 256, 128, 64, 32, 16, 8, 4, 2, 1]
        # self.compilation_config.init_with_cudagraph_sizes(hardcoded_sizes)
